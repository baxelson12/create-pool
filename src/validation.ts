...
export const poolCreationSchema = z.object({
  ...
  currency0Price: z.number().positive({ message: 'Price must be a positive number.' }),
  currency1Price: z.number().positive({ message: 'Price must be a positive number.' }),
  fee: z.number().int().positive({ message: 'Fee must be a positive integer.' }),
  tickSpacing: z.number().int().positive({ message: 'Tick spacing must be a positive integer.' }),
